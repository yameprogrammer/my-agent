...
## 5. 다중 LLM 제공자 연동 설계 (Multi-LLM Provider Architecture)
사용자가 소설 프로젝트별로 사용할 AI API(OpenAI, Google Gemini, Anthropic Claude, Ollama 등)를 자유롭게 선택할 수 있도록 설계한다.

### 5.1 LLM 팩토리 클래스 (LLMFactory) 구조
각 에이전트 노드들은 직접 LLM 인스턴스를 생성하지 않고, 프로젝트에 세팅된 설정값을 바탕으로 `LLMFactory`를 통해 알맞은 LangChain `BaseChatModel` 객체를 주입받는다.
...
class LLMFactory:
    ...
    def get_model(
        ...
        if provider_lower == "openai":
            ...
            # Ollama는 로컬 호스트에 띄워진 API 세팅을 사용 (기본 11434 포트)
            return ChatOllama(
                model=model_name,
                temperature=temperature
            )
...
### 5.2 프로젝트 LLM 설정 운용 규칙
1. **API Key 관리 가이드라인**:
   - `api_key_override`가 `NULL`일 경우, 서버 전역 환경변수(`.env`에 등록된 `OPENAI_API_KEY`, `GOOGLE_API_KEY` 등)를 기본 인증 정보로 사용한다.
   - 사용자가 자신만의 개인 API Key를 사용하고자 할 경우 `api_key_override`에 저장하고, 팩토리가 우선적으로 이를 적용한다.
2. **에이전트별 특화 모델 배정**:
   - 비용과 속도 효율성을 위해 씬 집필(`Writer`)은 `gpt-4o-mini`나 로컬 `llama3`, 일관성 검증(`Judge`) 및 시놉시스 설계(`Plotter`)는 상위 등급의 고성능 모델(`gpt-4o`, `gemini-1.5-pro` 등)로 개별 설정하여 연동할 수 있도록 데이터 모델을 지속 확장한다.
```
