...
- **수행 태스크** (구간 커밋):
  - IDEA-02/03/09 캐릭터 상태·복선·작가 메모
  - IDEA-04/05/08/13 아크 플래너·말미 훅·스타일·저비용
  - IDEA-06/07/10/23 씬 재집필·체크포인트·취소·프로젝트 락
  - IDEA-11/12/14/15 사용량·투고 프리셋·회차 다운로드
  - IDEA-16/17/18/19 import·그래프·공유·템플릿
  - IDEA-21 agents 패키지 분리
  - SPA: 설정/대시보드/세계관/집필실/다운로드 UI
- **보류**: IDEA-20 프롬프트 A/B DB, IDEA-22 TypeScript 전면화
- **Handoff**: IDEA 제품 기능 MVP 완료. 다음 가치는 실사용 피드백·IDEA-20/22 또는 장편 심화(상태 LLM 자동 갱신 등).
...
- **수행 태스크**:
  - [x] `BrainstormAgent` 프롬프트: 피드백이 기존 항목 변경을 요구하면 동일 keyword/name + `change_type=update` 로 완결 교체본 출력
  - [x] 스키마에 `change_type` / `change_summary` 추가, API가 기존 DB 키와 대조해 create/update 확정
  - [x] 프론트: 미적용 추천안을 다음 생성 컨텍스트로 재전송, `✏️ 기존 수정` 배지·적용 시 upsert 카운트 표시
- **동작 요약**: 생성만으로는 DB 미변경. 사용자가 수정안을 체크 후 적용하면 동일 이름/키워드 행이 업데이트됨.
